...
import spaceinvader.entity.InvaderCaretakerMemento;
import spaceinvader.entity.InvaderMemento;
import spaceinvader.entity.InvaderOriginatorMemento;
 ...
 */
public class GameInvaderImpl implements Game, Observer {
	...
	private GameLevelInvaderImpl currentPlayedLevel = null;
	...
	private InvaderOriginatorMemento originator=new InvaderOriginatorMemento();
	private InvaderCaretakerMemento caretaker=new InvaderCaretakerMemento();
	public GameInvaderImpl() {
	...
	public void start() {
		...
		for (GameLevel level : gameLevels) {
			...
			try {
				...
				currentPlayedLevel = (GameLevelInvaderImpl) level;
				levelNumber++;
	...
	public synchronized void restore() {
		//GameLevelInvaderImpl level = currentPlayedLevel;
		currentPlayedLevel.suspend();
		//currentPlayedLevel.interrupt();
		//currentPlayedLevel = null;
		originator.restoreFromMemento( (InvaderMemento) caretaker.getMemento(0) );
		//currentPlayedLevel = level;
		currentPlayedLevel.setUniverse(originator.saveToMemento().getSavedState());
	...
	public synchronized void save() {
		currentPlayedLevel.suspend();
		originator.set(currentPlayedLevel.getUniverse());
